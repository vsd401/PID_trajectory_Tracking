...
class Simulator():
    ...
    def Sim(self, ClosedLoopData, Controller):
        ...
        for i in range(0, int(ClosedLoopData.Points)):
            Controller.solve(x[i, :])  # x为Curvature下X状态量 【vx vy wz phi X Y】

            u[i, :] = Controller.uPred[0, :]


            x[i + 1, :], x_glob[i + 1, :] = _DynModel(x[i, :], x_glob[i, :], u[i, :], np, ClosedLoopData.dt,
                                                      self.map.PointAndTangent)  # x, x_glob 分别为Curvature和global下X状态量
            '''x 为【vx vy wz phi_cur X_cur Y_cur】 
               x_global为【vx vy wz phi X Y】'''
            SimulationTime = i + 1
            ...
            if i <= 5:
                print("Linearization time: %.4fs Solver time: %.4fs" % (
                    Controller.linearizationTime.total_seconds(), Controller.solverTime.total_seconds()))
...
class PID:
    ...
    def solve(self, x0):
        ...
        """
        vt = self.vt
        self.uPred[0, 0] = - 0.6 * x0[5] - 0.9 * x0[3] + np.max(
            [-0.9, np.min([np.random.randn() * 0.25, 0.9])])  # x0[3]为横摆误差？ x0[5]为侧向跟踪误差
...
def _DynModel(x, x_glob, u, np, dt, PointAndTangent):
    ...
    while (i + 1) * deltaT <= dt:
        ...
        cur = Curvature(s, PointAndTangent)  # 这段轨迹的曲率
