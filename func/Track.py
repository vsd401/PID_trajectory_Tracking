...
class Map():
    ...
    def __init__(self, halfWidth):
        ...
        # Now given the above segments we compute the (x, y) points of the track and the angle of the tangent vector
        # (psi) at these points. For each segment we compute the (x, y, psi) coordinate at the last point of the
        # segment. Furthermore, we compute also the cumulative s at the starting point of the segment at signed
        # curvature PointAndTangent = [x, y, psi, cumulative s, segment length, signed curvature]
        #                                         cumulative s :片段长度, segment length：起点到目前端点的长度
        ...
        for i in range(0, PointAndTangent.shape[0]):
            ...
            if PointAndTangent[i, 5] == 0.0:  # If segment is a straight line
                ...
                if (la.norm(np.array([xs, ys]) - np.array([x, y]))) == 0:
                    ...
                    if np.abs(computeAngle([x, y], [xs, ys], [xf, yf])) <= np.pi / 2 and np.abs(
                            computeAngle([x, y], [xf, yf], [xs, ys])) <= np.pi / 2:
